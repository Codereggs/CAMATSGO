...
export default function CRUDtransacciones(
  deleteBtn,
  importeDEP,
  nroTarjetaDEP,
  CCVDEP,
  ...
  d.addEventListener("click", (e) => {
    ...
    if (e.target === $confBtnDEP) {
      ...
      if (primeraTransaccion) {
        if (parseInt($CCVDEP.value) !== datosUsuario.securityCode)
          return alert(
            "Su transacción no puede ser procesada, por favor verifique su código de seguridad."
          );
        //POST
        try {
          const primeraTrans = async () => {
            ...
            if (resp.ok)
              return alert("Su primer depósito fué realizado con éxito.");

            if (!resp.ok) {
              if(resp.statusText === "Bad Request") {alert("No puede realizar un depósito menor a $1000.")}
              throw { status: resp.status, statusText: resp.statusText };
            }
          };
        ...
        try {
          const depositoARS = async () => {
            ...
            let resp = await fetch(
              ...
              json = await resp.json();
            if (resp.ok) return alert("Su depósito fué realizado con éxito.");
            if (!resp.ok){
              if(resp.statusText === "Bad Request") {alert("No puede realizar un depósito menor a $1000.")}
              throw { status: resp.status, statusText: resp.statusText };
            }};
    ...
    if (e.target === $botonRET) {
      try {
        const retiroARS = async () => {
          ...
          if (resp.ok) return alert("Su retiro fué realizado con éxito.");
          if (!resp.ok)
            throw { status: resp.status, statusText: resp.statusText };
        };
        //Ejecucion
    ...
    if (e.target === $btnBTCCOM) {
      if ($BTCCOM.value === "" || $pesosCOM.value === "")
        return alert("Inserte una cantidad válida para realizar su compra.");
      try {
    ...
    if (e.target === $btnBTCVEN) {
      if ($BTCVEN.value === "" || $pesosVEN.value === "")
        return alert("Inserte una cantidad válida para realizar su compra.");
      try {
        const ventaBTC = async () => {
          ...
          if (resp.ok) {
            alert("Su venta fue realizada con éxito.");
            $pesosVEN.value = "";
            $BTCVEN.value = "";
            return true;
          }
        ...
        console.log(`Error ${err.status}: ${message}`);
        alert(
          "Su saldo es insuficiente para realizar esta transacción. Ingrese una cantidad válida."
        );
      }
    }
  });
  ...
  try {
    const dameloTodo = async () => {
      let resp = await fetch(`https://apiusuarioscamatsgo.herokuapp.com/${ID}`),
        json = await resp.json();
    ...
    const traeloTodo = async (canva, pARS, pBTC) => {
      ...
      const data = {
        ...
        config = {
          ...
          data: data,
          options: {
            plugins: { legend: { labels: { color: "#12d400" } } },
...
export function precioBtc(id, BTCCOM, pesosCOM, BTCVEN, pesosVEN) {
  ...
  //Funcion para balancear compra - venta de bitcoins
  const estBalBTCARS = () => {
    let dec = /^\d*\.?\d*$/;
    d.addEventListener("change", (e) => {
      if (e.target === $BTCCOM) {
        ...
        if (
          dec.test($BTCCOM.value) === false ||
          Math.sign($BTCCOM.value) === -1
        ) {
          $BTCCOM.value = "";
          $pesosCOM.value = "";
      ...
      if (e.target === $pesosCOM) {
        ...
        if (
          dec.test($pesosCOM.value) === false ||
          Math.sign($pesosCOM.value) === -1
        ) {
          $BTCCOM.value = "";
          $pesosCOM.value = "";
          return alert("Inserta un número correcto.");
      ...
      if (e.target === $BTCVEN) {
        ...
        if (
          dec.test($BTCVEN.value) === false ||
          Math.sign($BTCVEN.value) === -1
        ) {
          $BTCVEN.value = "";
          $pesosVEN.value = "";
          return alert("Inserta un número correcto.");
      ...
      if (e.target === $pesosVEN) {
        ...
        if (
          dec.test($pesosVEN.value) === false ||
          Math.sign($pesosVEN.value) === -1
        ) {
          $BTCVEN.value = "";
          $pesosVEN.value = "";
          return alert("Inserta un número correcto.");
