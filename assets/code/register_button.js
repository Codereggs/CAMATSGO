...
export default function registerBtn (DOMclass,email,name,id,phone,pass,confPass) {
...
const activarBoton = () => $botonEnviar.removeAttribute("disabled");
    ...
    d.addEventListener("click", (e) => {
      ...
      if (campoVacio!="") mensajeError = `Los siguientes campos requeridos están vacíos: ${campoVacio+mensajeError} `; 
  
      //Validamos el formato del email
      if(isEmail($email.value) === false && $email.value != "") mensajeError +=`Tu dirección de email ${$email.value} no es válida.`;
      
      // Validación numérica del campo teléfono
      if(esNumero($phone.value) === false && $phone.value != "") mensajeError += `Tu teléfono no es válido.`;
      
      //Validación campo password y confirmación password
      if(/^(?=.*\d)(?=.*[\u0021-\u002b\u003c-\u0040])(?=.*[A-Z])(?=.*[a-z])\S{8,16}$/.test($pass.value) === false && $confPass.value != "") mensajeError += `La contraseña debe tener entre 8 y 16 caracteres, al menos un dígito, al menos una minúscula, al menos una mayúscula y al menos dos caracter no alfanumérico`; 
        ...
        if(mensajeError!="")
      {
        ...
        //Almacenar en symbol
        usuario[symbolU] = [$name.value,$id.value,$email.value,$phone.value];
